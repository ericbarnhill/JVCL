...
__kernel void spinFact(__global float* w, int n) {	
	unsigned int i = get_global_id(0);
	if (i % 2 == 0) {
		w[i] = cos(2*i*PI/(float)n);
	} else {
		w[i] = cos(2*i*PI/(float)(n+PI_2));
	}
}	
	
__kernel void bitReverse(__global float *dst, __global float *src, int m, int n)	{	
...
__kernel void norm(__global float *x, int n)	
{	
unsigned int gid = get_global_id(0);	
unsigned int nid = get_global_id(1);	
	
x[nid*n+gid] = x[nid*n+gid] / (float)n;	
}	
	
__kernel void butterfly(__global float *r, __global float *i, __global float* w, int m, int n, int iter, uint flag)	
{	
unsigned int gid = get_global_id(0);	
unsigned int nid = get_global_id(1);	
