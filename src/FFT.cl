...
wab = as_float2(as_uint2(w[l]) ^ (uint2)(0x0, (uint)flag));	
...
__kernel void butterfly2(__global float2 *x, __global float2* w, int m, int n, int iter, int flag)	
{
unsigned int gid = get_global_id(0);	
unsigned int nid = get_global_id(1);	
